...
void add_edge(int from, int to, int cap) {
	G[from].push_back((edge) {to, cap, (int)G[to].size() }); // 从from到to容量cap 反向边为G[to].size()
	G[to].push_back((edge) {from, 0, (int)G[from].size()-1 }); // 从to到from容量0 反向边为G[from].size()-1
}
...
int main() {
	...
	for(int i = m + 1; i <= n; i++) add_edge(i, ed, 1);		
